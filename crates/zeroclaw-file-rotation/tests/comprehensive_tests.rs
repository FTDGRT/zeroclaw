...
async fn date_rotation_rotates_old_file() {
    ...
    let yesterday_date = (chrono::Local::now() - chrono::Duration::seconds(24 * 3600)).date_naive();
    ...
    let rotated: Vec<_> = std::fs::read_dir(tmp.path())
        ...
        .filter(|e| {
            ...
            name.contains(&yesterday_str) && name != "date-rotate.jsonl"
        })
        .collect();

    assert!(
        !rotated.is_empty(),
        "Expected a dated rotated file with yesterday's date ({yesterday_str})"
    );
...
// ── 19. Date rotation names file with mtime date, not current date ──

#[tokio::test]
async fn date_rotation_uses_mtime_date_in_filename() {
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("mtime-test.jsonl");
    ...
    let rotated: Vec<_> = std::fs::read_dir(tmp.path())
        ...
        .filter(|e| {
            let name = e.file_name().to_string_lossy().to_string();
            name.contains(&yesterday_str) && name != "mtime-test.jsonl"
        })
        .collect();

    assert!(
        !rotated.is_empty(),
        "Expected a rotated file with yesterday's date ({yesterday_str}) in the name"
    );

    // Verify old data is in the rotated file
    let rotated_contents = std::fs::read_to_string(rotated[0].path()).unwrap();
    assert!(rotated_contents.contains(r#"{"old":"data"}"#));
}

// ── 20. Size rotation names file with current date ──

#[tokio::test]
async fn size_rotation_uses_current_date_in_filename() {
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("size-date.log");
    ...
    // Write enough lines to trigger size rotation
    for i in 0..20 {
        writer
    ...
    // Verify all rotated files have today's date in the name
    let rotated: Vec<_> = std::fs::read_dir(tmp.path())
        .unwrap()
        .filter_map(|e| e.ok())
        .filter(|e| {
            let name = e.file_name().to_string_lossy().to_string();
            name != "size-date.log" && name.contains('.')
        })
        .collect();

    assert!(
        !rotated.is_empty(),
        "Expected at least one rotated file from size rotation"
    );

    for entry in &rotated {
        let name = entry.file_name().to_string_lossy().to_string();
        assert!(
            name.contains(&today_str),
            "Rotated file '{name}' should contain today's date ({today_str})"
        );
    }
}

// ── 21. Date rotation then size rotation in same session ──

#[tokio::test]
async fn date_then_size_rotation_combined() {
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("combined.jsonl");
    ...
    // Pre-create file with yesterday's mtime and enough content to trigger size rotation later
    std::fs::write(&path, "x".repeat(80)).unwrap();
    let yesterday = std::time::SystemTime::now() - Duration::from_secs(24 * 3600);
    filetime::set_file_mtime(&path, filetime::FileTime::from_system_time(yesterday)).unwrap();
    ...
    let all_files: Vec<String> = std::fs::read_dir(tmp.path())
        .unwrap()
        .filter_map(|e| e.ok())
        .map(|e| e.file_name().to_string_lossy().to_string())
        .collect();

    // Should have a rotated file named with yesterday's date (date rotation)
    let yesterday_files: Vec<_> = all_files
        .iter()
        .filter(|n| n.contains(&yesterday_str) && n != &"combined.jsonl")
        .collect();
    assert!(
        !yesterday_files.is_empty(),
        "Expected at least one file with yesterday's date ({yesterday_str}), found: {all_files:?}"
    );

    // Should have a rotated file named with today's date (size rotation)
    let today_files: Vec<_> = all_files
        .iter()
        .filter(|n| n.contains(&today_str) && n != &"combined.jsonl")
        .collect();
    assert!(
        !today_files.is_empty(),
        "Expected at least one file with today's date ({today_str}), found: {all_files:?}"
    );
}

// ── 22. Sequence increments correctly across dates ──

#[tokio::test]
async fn sequence_increments_across_dates() {
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("seq-dates.jsonl");
    ...
    let all_files: Vec<_> = std::fs::read_dir(tmp.path())
        .unwrap()
        .filter_map(|e| e.ok())
        .filter_map(|e| {
            let name = e.file_name().to_string_lossy().to_string();
            zeroclaw_file_rotation::parse_rotated_filename(&name, "seq-dates", "jsonl")
                .map(|p| (name, p.date, p.seq))
        })
        .collect();

    // Verify no duplicate (date, seq) pairs
    let mut seen = std::collections::HashSet::new();
    for (_, date, seq) in &all_files {
        let key = (*date, *seq);
        assert!(
            seen.insert(key),
            "Duplicate (date, seq) pair found in rotated files"
        );
    }
}

// ── 23. Double date rotation is idempotent ──

#[tokio::test]
async fn double_date_rotation_idempotent() {
    let tmp = tempfile::tempdir().unwrap();
    let path = tmp.path().join("idempotent.jsonl");
    ...
    // Create file with yesterday's mtime
    std::fs::write(&path, r#"{"first":"write"}"#).unwrap();
    let yesterday = std::time::SystemTime::now() - Duration::from_secs(24 * 3600);
    filetime::set_file_mtime(&path, filetime::FileTime::from_system_time(yesterday)).unwrap();
    ...
    // Active file should contain only the new writes
    let active = std::fs::read_to_string(&path).unwrap();
    assert!(active.contains(r#"{"second":"write"}"#));
    assert!(active.contains(r#"{"third":"write"}"#));
    ...
    let rotated: Vec<_> = std::fs::read_dir(tmp.path())
        ...
        .filter(|e| {
            let name = e.file_name().to_string_lossy().to_string();
            name != "idempotent.jsonl"
        })
        .collect();
    assert_eq!(
        rotated.len(),
        1,
        "Expected exactly 1 rotated file, got {}",
        rotated.len()
    );
