...
impl RotatingFileWriter {
    ...
    /// Flush all pending appends to disk without shutting down the writer.
    ///
...
mod tests {
    ...
    #[tokio::test]
    async fn flush_ensures_all_appends_on_disk() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("flush_test.jsonl");
        let writer = RotatingFileWriter::new(
            path.clone(),
            RotationConfig {
        ...
        // All 50 lines must be on disk after flush
        let contents = std::fs::read_to_string(&path).unwrap();
        ...
        assert_eq!(
            ...
            51,
            "51 lines after post-flush append"
        );
        assert!(contents.lines().any(|l| l == "after-flush"));
    }

    #[tokio::test]
    async fn flush_does_not_close_writer() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("flush_alive.jsonl");
        let writer = RotatingFileWriter::new(
            path.clone(),
            RotationConfig {
                max_file_size_bytes: 1024 * 1024,
        ...
        assert_eq!(lines, vec!["before-flush", "after-flush", "final"]);
    }

    #[tokio::test]
    async fn flush_then_shutdown_is_safe() {
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("flush_then_shutdown.jsonl");
        let writer = RotatingFileWriter::new(
            path.clone(),
            RotationConfig {
                max_file_size_bytes: 1024 * 1024,
