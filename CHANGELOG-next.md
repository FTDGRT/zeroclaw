...
- **Conversation attribution signatures changed**: `zeroclaw_runtime::agent::loop_::process_message`'s `conversation_id` parameter semantics tightened (callers must mint their own id rather than passing `None` for detached deliveries); `agent_turn` and `TurnAttribution` callers must supply a server-resolved conversation id, never a raw caller-supplied session identifier; and `SessionBackend` implementors must now provide `resolve_or_create_conversation_id` and `clear_and_rotate_conversation` (no default implementations - every backend must declare its behavior explicitly, because a "fresh UUID every call" default would violate the cross-turn stability contract, and crash/transaction semantics differ per backend). `zeroclaw-infra` and `zeroclaw-runtime` are Beta/Experimental per the stability grading in `docs/book/src/contributing/agent-guidelines.md`, so this ships within a MINOR version with this changelog note as the only migration guidance: implementors add the two new trait methods (no safe default exists); callers pass an explicit id instead of relying on a raw session identifier.
